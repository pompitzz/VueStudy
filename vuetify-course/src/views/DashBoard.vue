<template>
    <div>
        <h1 class="v-subheader grey--text">Dashboard</h1>

        <v-container class="my-5">
            <v-row class="mb-3">
                <v-tooltip top>
                    <template v-slot:activator="{on}">
                        <v-btn small text class="grey mr-3 lighten-3" @click="sortBy('title')" v-on="on">
                            <v-icon left small>mdi-folder-marker</v-icon>
                            <span class="caption text-lowercase">My Project Name</span>
                        </v-btn>
                    </template>
                ...
                <v-tooltip top>
                    <template v-slot:activator="{on}">
                    <v-btn small text class="grey mr-3 lighten-3" @click="sortBy('person')" v-on="on">
                ...
                <v-row :class="`pa-3 project ${project.status}`">
                    ...
                    <v-col cols="2" sm="4" md="2">
                        <div class="text-center">
                            <v-chip small :class="`${project.status}`" class="white--text caption my-2">
                                {{project.status}}
                            </v-chip>
                        </div>
                    </v-col>
