<template>
    <div>
        <h1 class="v-subheader grey--text">Team</h1>
        <v-container class="my-5">
            <v-row>
                <v-col cols="12" sm="6" md="4" lg="3" v-for="person in team" :key="person.name">
                    <v-card flat class="text-center ma-3">
                        <v-responsive class="pt-4">
                            <v-avatar size="100" class="grey lighten-2" outlined>
                                <img :src="person.avatar"/>
                            </v-avatar>
                        </v-responsive>
                        <v-card-text class="text-centere">
                            <div >{{person.name}}</div>
                            <div class="grey">{{person.role}}</div>
                        </v-card-text>
                        <v-card-actions>
        ...
        data() {
            return {
                team: [
                    ...
                    {name: 'Bill Gates', role: 'Director', avatar: '/avatar-5.png'},
                ]
            }
        },
